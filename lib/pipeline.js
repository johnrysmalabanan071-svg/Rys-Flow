...
export function configuration(env=process.env) {
 ...
 if(missing.length){
  console.error('[portfolio-config] Missing required variables: '+missing.join(', '));
  throw new HttpError(503,'The inquiry service is not ready yet. Please use the email link.');
 }
 const issues=[];
 const https=value=>{try{return new URL(value).protocol==='https:';}catch{return false;}};
 if(!https(env.SUPABASE_URL))issues.push('SUPABASE_URL must be a valid HTTPS URL');
 if(env.RATE_LIMIT_SECRET.length<32)issues.push('RATE_LIMIT_SECRET must contain at least 32 characters');
 if(env.CRON_SECRET.length<32)issues.push('CRON_SECRET must contain at least 32 characters');
 if(Boolean(env.LEAD_WEBHOOK_URL)!==Boolean(env.LEAD_WEBHOOK_SECRET))issues.push('LEAD_WEBHOOK_URL and LEAD_WEBHOOK_SECRET must both be set, or both omitted');
 if(env.LEAD_WEBHOOK_URL&&!https(env.LEAD_WEBHOOK_URL))issues.push('LEAD_WEBHOOK_URL must be a valid HTTPS URL');
 if(issues.length){
  // Only fixed diagnostic messages are logged, never values or visitor details.
  console.error('[portfolio-config] '+issues.join('; '));
  throw new HttpError(503,'The inquiry service is temporarily unavailable. Please use the email link.');
 }
 return env;
}
